...
The [surviving-company rename propagation](docs/v2_SURVIVING_RENAME_PROPAGATION.md) now qualifies all feature dependencies of source-clocked SSBR→ALSO and ARZZ→AZZA, retaining55/60new eligible days and all933/full60history. Earlier daily, issuer, lending and auxiliary proofs are reused; native M1/scalars and magnitude/cross-market now pass877 actual consumer samples and independent arithmetic. No predecessor M1 stream exists for SSBR/ARZZ, so missing history remains missing. New market-family losses are398 retired predecessor fields; earlier226 live unit-uncertainty and620 daily peer losses remain explicit. Both accepted stores and old fits stay sealed. Final primary-target/new complete-store composition, January25 disposition and integratedStageA remain before C/D; no loan alias, neural forward or model result. This remains the existing composition block.
