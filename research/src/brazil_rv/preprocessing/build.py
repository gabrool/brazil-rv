...
def _build_causal_tod_profile(
    output_dir: Path,
    source_groups: list[pl.DataFrame],
    market_dates: tuple[object, ...],
    bar_assignment_dates: dict[str, frozenset[object]],
    membership: np.ndarray,
    slot_by_security: dict[str, int],
) -> np.ndarray:
    total = np.zeros((len(market_dates), PROFILE_BIN_COUNT), dtype=np.float64)
    ...
    for source_number, group in enumerate(source_groups, start=1):
        ...
        allowed_dates = frozenset().union(
            *(
                bar_assignment_dates[security_id]
        ...
        for assignment in group.iter_rows(named=True):
            ...
            bars = session_bars.filter(
                pl.col("trade_date").is_in(tuple(bar_assignment_dates[security_id]))
            )
            ...
            legacy = build_equity_features(
                raw_grid, observed, identity_day, market_dates=market_dates
            )
            moments = daily_close_move_statistics(
                raw_grid,
                observed,
                membership[:, slot_by_security[security_id]],
                legacy.data_ready,
                legacy.sigma,
            )
            total += moments[0]
        ...
        if source_number % 20 == 0 or source_number == len(source_groups):
            print(f"Collected TOD statistics {source_number}/{len(source_groups)}")
    profile = estimate_causal_profile(
        variance_from_sufficient_statistics(total, total_sq, count),
        count,
        market_dates,
    )
    write_profile_artifact(output_dir, profile, market_dates)
    return profile.relative_variance
...
def _populate_feature_store(
    ...
    tod_profile = _build_causal_tod_profile(
        output_dir,
        source_groups,
        market_dates,
        bar_assignment_dates,
        membership,
        slot_by_security,
    )
    ...
    for source_number, group in enumerate(source_groups, start=1):
        ...
        for assignment in group.iter_rows(named=True):
            ...
            tod_dynamic, tod_valid = build_full_tod_dynamic_features(
                raw_grid, observed, result.data_ready, result.sigma, tod_profile
            )
            arrays["equity_features.npy"][:, slot] = tod_dynamic
            arrays["equity_slow.npy"][:, slot] = result.slow
            arrays["equity_data_ready.npy"][:, slot] = result.data_ready
            dynamic_valid[:, slot] = tod_valid
...
def build_feature_store(*, created_at: datetime | None = None) -> tuple[Path, Path]:
    ...
    output_dir = (
        OUTPUT_BASE / f"m1_features_pit_causal_tod_{created_at:%Y%m%dT%H%M%S%fZ}"
    )
...
def _write_manifest(
    ...
    tod_profile = json.loads(
        (output_dir / "equity_tod_profile.json").read_text(encoding="utf-8")
    )
    manifest = {
        "contract_version": CONTRACT_VERSION,
