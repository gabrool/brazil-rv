...
class EvaluationInputs:
    ...
    action_settlements: tuple[ActionSettlement, ...] = ()
...
def _action_attribution(
    ...
    return {
        ...
        "action_settlements": [asdict(event) for event in inputs.action_settlements],
        "withholding_accrual": result.withholding_accrual.tolist(),
        "lender_compensation": result.lender_compensation.tolist(),
        "loan_cash_payments": [
...
def _input_hashes(inputs: EvaluationInputs) -> dict[str, str]:
    result = {
        ...
        "action_settlements": hashlib.sha256(
            json.dumps(
                [asdict(event) for event in inputs.action_settlements], sort_keys=True
            ).encode()
        ).hexdigest(),
...
def _economics_contract(inputs: EvaluationInputs) -> dict[str, object]:
    ...
    return {
        ...
        "spot_cash_lag_overrides": config.spot_cash_lag_overrides,
        "spot_delivery_lag_overrides": config.spot_delivery_lag_overrides,
        "loan_extra_accrual_after": config.loan_extra_accrual_after,
