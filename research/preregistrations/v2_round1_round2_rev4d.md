# Brazil-RV v2 Round 1 / Round 2 registration rev 4d

Status: frozen before any rev-4d score. Rev 4c is void for future research
decisions, while all of its artifacts remain immutable engineering evidence.
All folds, purges, clocks, targets, controls, construction ordering, settlement
rules, borrow rules, hedge rules, protected-window rules, and decision rules
remain as registered in rev 4c. This revision changes only the small-stratum
scaling threshold described below. No official-validation or permanently spent
test row may be read.

## Small-stratum threshold

Each volatility quintile's target quota and retention buffer remain six names
per side when the global side target and buffer are both thirty. Proportional
scaling now begins only when a quintile contains fewer than twice the combined
quota and buffer, rather than four times that width. The factor of two is the
exact disjointness requirement for the long and short bands and matches the
pre-stratification rule that each side's retention band may occupy at most half
the eligible population. Scaling below the threshold, quota remainder order,
within-quintile ranking, current-quintile retention, and global same-side spill
are unchanged.

## Acceptance and rounds

Acceptance is CPU-only and evaluates exactly the five registered controls on
F1/F2/F3 plus one F1 `b_intraday` GBDT: sixteen books. It retains all rev-4c
sanity, D1-D5, target-null, source, native-fast, BOVA11, realized-beta,
occupancy, and equity-gross gates. Every evaluation reports the mean entry-
eligible population, mean and minimum volatility-quintile sizes, mean quota
vector, gross-shortfall decomposition, all three borrow cells, hedge statistics,
rate coverage, and placeholder use.
