...
unresolved-action boundary, and the explicitly approved decoded history-age
provenance hash. All other hashes and report fields must match
...
The user separately approved exactly `history_age_sessions` in
`recomputed_input_hashes` after the Round-2 input-identity stop, with no additional
diagnostic exemptions. Report sealed and host hashes side by side. All quality
strata, their counts and ICs remain under exact comparison, as do all other
protected score-derived fields. The decoded-age formula and validity mask stay
unchanged for the rev4f replays. New frozen manifests explicitly carry this
decoding contract in the protocol. The completed Round-1 root retains its original
immutable manifest and implementation binding; it is not rewritten or replayed.

### Future diagnostic canonicalisation (not active in this replay)

From Round 1' on the rebuilt store, apply `np.rint` after the existing history-age
decode, preserve the validity mask, and hash canonical integer session counts.
Verify raw-versus-canonical history-age stratification on the current store before
that freeze. A decoded diagnostic provenance array is hashed after canonicalisation
to its natural type; model inputs, targets and scores remain hashed exactly as
stored. This future contract does not alter the current rev4f replay or R3.1's
sealed-panel diagnostic identity contract.
