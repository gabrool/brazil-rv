...
def test_dynamic_return_since_open_activates_at_delayed_first_observation() -> None:
    ...
    for position in (first_observed, first_observed + 5):
        expected = np.log(raw[20, position, 3] / raw[20, first_observed, 0]) / (
            baseline.sigma[20] * np.sqrt(position + 1)
        )
...
def _opening_audit_arrays(valid_count: int) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "equity_membership.npy": np.ones((2, equity_count), dtype=bool),
        "equity_data_ready.npy": np.ones((2, equity_count), dtype=bool),
    }


def test_opening_audit_reports_activation_and_conditionally_rejects_dead_rank() -> None:
    summary = opening_feature_family_stats(
        _opening_audit_arrays(30), np.array([1], dtype=np.int64)
    )
    ...
    rank = next(
        ...
        if row["feature"] == "overnight_gap_cross_section_rank"
    )
    ...
    with pytest.raises(ValueError, match="constant or entirely zero"):
