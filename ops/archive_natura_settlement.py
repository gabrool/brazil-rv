...
def main():
    ...
    bound_json(run["natura_settlement_acceptance"])
    audit = bound_json(run["natura_settlement_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"natura_settlement_{commit[:7]}.zip"
    restored = base / f"natura_settlement_recovery_{commit[:7]}"
    ...
    for rec in dependencies.values():
        bound_json(rec)
    evidence = base / "natura_settlement"
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_NATURA_ACCOUNT_SETTLEMENT.md",
            "docs/v2_natura_settlement_acceptance.json",
        ...
        paths += [
            ...
            for p in (PROJECT / "ops").glob("*natura_settlement.py")
        ]
    ...
    cells = 0
    cases = []
    for receipt in (audit, bound_json(run["natura_bonus_disposal_audit"])):
    ...
    assert len(cases) == 42
    assert cells == bound_json(run["natura_settlement_qualification"])["account_cells"]
    result = dict(
        status="verified",
        ...
        acceptance=run["natura_settlement_acceptance"],
        dependencies=dependencies,
        no_immutable_inputs_stores_or_old_fits_duplicated=True,
        seconds=perf_counter() - started,
    )
    output = base / "natura_settlement_recovery.json"
    write_json_atomic(output, result)
    run["natura_settlement_recovery"] = binding(output)
