"""Archive and restore ordinary monthly custody bounds and four new originals, with sealed inputs as dependencies."""
...
def main():
    ...
    accepted = bound_json(run["custody_fee_acceptance"])
    audit = bound_json(run["custody_fee_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"custody_fees_{commit[:7]}.zip"
    restored = base / f"custody_fee_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_CUSTODY_FEES.md",
            "docs/v2_custody_fee_acceptance.json",
            "docs/v2_economic_data_scaling_run.json",
            "docs/v2_economic_data_scaling_progress.md",
            "ops/verify_corporate_replay.py",
        ]
        paths += [
            "ops/recover_custody_tariffs.py",
        ]
        paths += [
            p.relative_to(PROJECT).as_posix()
            for p in (PROJECT / "ops").glob("*custody_fee*.py")
        ]
        ...
        for folder in ("custody_fees", "custody_tariff_sources"):
    ...
    cells = 0
    for case in cases:
        folder = restored / "evidence" / "custody_fees" / case["book"]
    ...
    sources = restored / "evidence/custody_tariff_sources"
    receipts = json.loads(
        (sources / "boundary_sources.json").read_text(encoding="utf8")
    )
    ...
    report = dict(
        ...
        acceptance=run["custody_fee_acceptance"],
        dependencies=dependencies,
        seconds=perf_counter() - tick,
        no_immutable_input_store_or_old_fit_copies=True,
    )
    output = base / "custody_fee_recovery.json"
    write_json_atomic(output, report)
    run["custody_fee_recovery"] = binding(output)
