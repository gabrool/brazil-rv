...
def main():
    ...
    bound_json(run["enat_settlement_acceptance"])
    audit = bound_json(run["enat_settlement_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"enat_settlement_{commit[:7]}.zip"
    restored = base / f"enat_settlement_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_ENAT_FRACTION_SETTLEMENT.md",
            "docs/v2_enat_settlement_acceptance.json",
            "docs/v2_economic_data_scaling_run.json",
            "docs/v2_economic_data_scaling_progress.md",
            "ops/verify_corporate_replay.py",
            "ops/recover_remaining_held_sources.py",
        ]
        paths += [
            p.relative_to(PROJECT).as_posix()
            for p in (PROJECT / "ops").glob("*enat_settlement.py")
        ]
        ...
        for family in ("enat_settlement", "remaining_held_sources"):
            for path in sorted((base / family).rglob("*")):
                if path.is_file():
                    add(
    ...
    for receipt in (audit,):
        complete = bound_json(receipt["completed"])
        label = Path(receipt["audit_root"]).name
        for case in complete:
            path = restored / "evidence" / label / case["book"] / "account.npz"
            with np.load(path) as a:
                assert a["signed_shares"].shape == (case["sessions"], 933)
                cells += sum(a[k].size for k in a.files)
        cases.extend(complete)
    assert len(cases) == 18
    assert (
        cells == bound_json(run["enat_settlement_qualification"])["account_array_cells"]
    )
    result = dict(
        status="verified",
        ...
        acceptance=run["enat_settlement_acceptance"],
        dependencies=dependencies,
        no_immutable_inputs_stores_or_old_fits_duplicated=True,
        seconds=perf_counter() - started,
    )
    output = base / "enat_settlement_recovery.json"
    write_json_atomic(output, result)
    run["enat_settlement_recovery"] = binding(output)
