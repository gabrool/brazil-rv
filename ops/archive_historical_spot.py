"""Archive and restore dated spot bounds and three new originals, with sealed inputs as dependencies."""
...
def main():
    ...
    accepted = bound_json(run["historical_spot_acceptance"])
    audit = bound_json(run["historical_spot_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"historical_spot_{commit[:7]}.zip"
    restored = base / f"historical_spot_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_HISTORICAL_SPOT_COSTS.md",
            "docs/v2_historical_spot_acceptance.json",
            "docs/v2_economic_data_scaling_run.json",
            "docs/v2_economic_data_scaling_progress.md",
            "ops/verify_corporate_replay.py",
        ]
        paths += [
            "ops/recover_historical_tariffs.py",
            "ops/inspect_cielo_saved_exposure.py",
        ]
        paths += [
            p.relative_to(PROJECT).as_posix()
            for p in (PROJECT / "ops").glob("*historical_spot*.py")
        ]
        ...
        for folder in ("historical_spot", "historical_tariff_sources"):
    ...
    cells = 0
    for case in cases:
        folder = restored / "evidence" / "historical_spot" / case["book"]
    ...
    assert len(cases) == 30 and cells == accepted["account_array_cells"]
    sources = restored / "evidence/historical_tariff_sources"
    receipts = json.loads((sources / "sources.json").read_text(encoding="utf8"))
    assert len(receipts) == 3
    for receipt in receipts:
        data = (sources / (receipt["id"] + ".pdf")).read_bytes()
    ...
    report = dict(
        ...
        restored_original_pdfs=len(receipts),
        acceptance=run["historical_spot_acceptance"],
        dependencies=dependencies,
        seconds=perf_counter() - tick,
        no_immutable_input_store_or_old_fit_copies=True,
    )
    output = base / "historical_spot_recovery.json"
    write_json_atomic(output, report)
    run["historical_spot_recovery"] = binding(output)
