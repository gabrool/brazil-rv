"""Continue four auxiliary families over the two sourced surviving renames."""
...
def main():
    ...
    new_links = [
        x
        for x in links
        if isins[x["predecessor_index"]] in ("BRSSBRACNOR1", "BRARZZACNOR3")
    ]
    ...
    write_json_atomic(
        ...
        dict(
            ...
            prior=run["remaining_auxiliaries"],
            contrast="Only two surviving-company spot links: bounded22-prior/22-following activity/options and six-prior oddlot history; new eligibility, original source assignment/availability, old thresholds and fields. Index original preview/last-effective identities and priorADV20. No option-series/loan alias, raw census or heldout consumer.",
    ...
    portfolios = selected(
        snapshots["output"], pl.col("disclosure_date") >= date(2019, 4, 1)
    )
    portfolios.write_parquet(out / "index_portfolios.parquet")
    index_cash = selected(
        cash_rec, pl.col("source_trade_date") >= date(2019, 1, 1)
    ...
    report = dict(
        ...
        limits="Four family intermediates; no raw census/source retrieval, option-series alias, loan alias, native/scalar/magnitude/crossmarket/target/fullstore/model result. Pre-Nov2019 IN/PR remains unsupported; empty schemas are not observations.",
    )
