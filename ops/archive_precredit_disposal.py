"""Archive and restore new precredit evidence, with immutable inputs as dependencies."""
...
def main():
    ...
    accepted = bound_json(run["precredit_disposal_acceptance"])
    audit = bound_json(run["precredit_disposal_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"precredit_disposal_{commit[:7]}.zip"
    restored = base / f"precredit_disposal_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_PRECREDIT_DISPOSAL.md",
            "docs/v2_precredit_disposal_acceptance.json",
        ...
        paths += [
            ...
            for p in (PROJECT / "ops").glob("*precredit_disposal.py")
        ]
        ...
        for path in sorted((base / "precredit_disposal").rglob("*")):
            if path.is_file():
                add("evidence/" + path.relative_to(base).as_posix(), path.read_bytes())
    ...
    report = dict(
        ...
        acceptance=run["precredit_disposal_acceptance"],
        dependencies=dependencies,
        seconds=perf_counter() - tick,
        no_immutable_input_store_or_old_fit_copies=True,
    )
    output = base / "precredit_disposal_recovery.json"
    write_json_atomic(output, report)
    run["precredit_disposal_recovery"] = binding(output)
