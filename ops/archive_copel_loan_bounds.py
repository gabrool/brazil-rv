"""Archive and restore new Copel bounds, with sealed inputs as dependencies."""
...
def main():
    ...
    accepted = bound_json(run["copel_loan_bounds_acceptance"])
    audit = bound_json(run["copel_loan_bounds_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"copel_loan_bounds_{commit[:7]}.zip"
    restored = base / f"copel_loan_bounds_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_COPEL_LOAN_BOUNDS.md",
            "docs/v2_copel_loan_bounds_acceptance.json",
        ...
        paths += [
            ...
            for p in (PROJECT / "ops").glob("*copel_loan_bounds.py")
        ]
        ...
        for path in sorted((base / "copel_loan_bounds").rglob("*")):
            if path.is_file():
    ...
    cells = 0
    for case in cases:
        folder = restored / "evidence" / "copel_loan_bounds" / case["book"]
        with np.load(folder / "account.npz") as arrays:
            assert arrays["signed_shares"].shape == (30, 933)
    ...
    report = dict(
        ...
        acceptance=run["copel_loan_bounds_acceptance"],
        dependencies=dependencies,
        seconds=perf_counter() - tick,
        no_immutable_input_store_or_old_fit_copies=True,
    )
    output = base / "copel_loan_bounds_recovery.json"
    write_json_atomic(output, report)
    run["copel_loan_bounds_recovery"] = binding(output)
