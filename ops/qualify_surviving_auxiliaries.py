"""Independent arithmetic on saved, dated surviving-rename source subsets."""
...
def main():
    ...
    saved = out / "reducer_results.json"
    ...
    for r in changed.iter_rows(named=True):
            ...
            while True:
                edge = next(
                    (
                        x
                        for x in links
                        if isins[x["predecessor_index"]] == isin
        ...
        weights = [
            sum(w for (_, name), w in x["weights"].items() if route(name) == r["isin"])
