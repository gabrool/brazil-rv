"""Archive and restore new payment bounds, with sealed inputs as dependencies."""
...
def main():
    ...
    accepted = bound_json(run["payment_bounds_acceptance"])
    audit = bound_json(run["payment_bounds_audit"])
    commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=PROJECT, text=True
    ).strip()
    base = Path(run["root"])
    archive = base / f"payment_bounds_{commit[:7]}.zip"
    restored = base / f"payment_bounds_recovery_{commit[:7]}"
    ...
    with zipfile.ZipFile(
        ...
        paths += [
            "PROJECT_CONTEXT.md",
            "docs/v2_PAYMENT_BOUNDS.md",
            "docs/v2_payment_bounds_acceptance.json",
        ...
        paths += [
            ...
            for p in (PROJECT / "ops").glob("*payment*.py")
        ]
        ...
        for folder in ("payment_bounds", "payment_bounds_initial"):
    ...
    cells = 0
    for case in cases:
        folder = restored / "evidence" / "payment_bounds" / case["book"]
    ...
    report = dict(
        ...
        acceptance=run["payment_bounds_acceptance"],
        dependencies=dependencies,
        seconds=perf_counter() - tick,
        no_immutable_input_store_or_old_fit_copies=True,
    )
    output = base / "payment_bounds_recovery.json"
    write_json_atomic(output, report)
    run["payment_bounds_recovery"] = binding(output)
