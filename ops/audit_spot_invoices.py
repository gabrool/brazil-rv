"""Six frozen invoice variants; all unrounded parent books are reused."""
...
def main():
    ...
    parents = []
    for kind in ("historical", "corporate"):
        window = plan["new_books"][kind]
        audit = bound_json(window["parent"])
        for case in bound_json(audit["completed"]):
            if case["start"] != window["start"] or case["scenario"] != "primary":
                continue
            if kind == "corporate" and case["sign"] != 1:
                continue
    ...
    write_json_atomic(
        ...
        dict(
            ...
            contract="Only invoice convention changes; saved unrounded parents are not replayed.",
    ...
    for parent in parents:
        ...
        case, kind = parent["case"], parent["kind"]
        ...
        config = LedgerConfig(**cfg, spot_invoice_convention="security_day_6dp_cent")
        original_config = asdict(config)
        original_config.pop("spot_invoice_convention")
        if "custody_claim_fraction" not in meta["provenance"]["config"]:
            assert (
                kind == "historical"
                and original_config.pop("custody_claim_fraction") == 1
            )
        ...
        scores = np.sin(
            ...
            + np.arange(len(rows))[:, None, None]
            * (0.07 if kind == "historical" else 0.003)
            + np.arange(5)[None, None, :] * 0.2
        ).astype(np.float32)
        if kind == "corporate":
            scores[:, [isins.index(x) for x in case["focus_isins"]], :] = 4
        base = replace(
            inputs,
            scores=scores,
        ...
        save_book(
            ...
            0,
            dict(
                policy="frozen_parent_spot_invoice_variant",
                scenario="base",
        ...
        with np.load(parent["account"]["path"]) as old:
            changed = np.flatnonzero(
                np.any(result.spot_invoice_adjustment != 0, axis=1)
            )
        ...
        report = dict(
            ...
            direct_invoice_adjustment=result.spot_invoice_adjustment.sum(0).tolist(),
